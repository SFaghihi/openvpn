...
#define SOCK_MAX_RND_SIZE 20
#define SOCK_MIN_RND_SIZE 1

#define SOCK_CRYPT_TAG_SIZE (256/8)
#define SOCK_CRYPT_BLOCK_SIZE (128/8)
#define SOCK_CRYPT_IV_SIZE (128/8)

#define SOCK_CRYPT_OFF_IV 0
#define SOCK_CRYPT_OFF_TAG (SOCK_CRYPT_OFF_IV + SOCK_CRYPT_IV_SIZE)
#define SOCK_CRYPT_OFF_ED (SOCK_CRYPT_OFF_TAG + SOCK_CRYPT_TAG_SIZE)

/**
 * Uniform random number in a range
 */
static inline
uint8_t random_uniform(uint8_t upper_bound)
{
 ...
 */
void socket_crypt_init_key(struct key_ctx_bi *key, const char *key_file,
 ...
 * tls_crypt_wrap().
 */
int socket_crypt_buf_overhead(void);

/**
 * Adjust frame parameters for --tls-crypt overhead.
 */
void socket_crypt_adjust_frame_parameters(struct frame *frame);
 ...
 * @param opt   The key state for this --socket-crypt instance.
 *
 * @returns true iff wrapping succeeded.
 */
bool socket_crypt_wrap(const struct buffer *src, struct buffer *dst,
 ...
 * no replay).
 */
bool socket_unwrap(const struct buffer *src, struct buffer *dst,
                      struct key_ctx_bi *key);
